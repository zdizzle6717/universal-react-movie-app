	...
	render() {
		...
		return (
			<div className="validate-error-element">
				<input className={validationClasses} name={this.props.name} type="checkbox" checked={this.state.checked} onClick={this.handleMouseDown} onChange={this.validateInputChange} onFocus={this.handleFocus} onBlur={this.handleBlur} disabled={this.props.disabled}/>
				<label className={this.props.required ? 'required' : ''} htmlFor={this.props.name} onClick={this.validateInputChange}>{this.props.label}</label>
			</div>
		)
	}
}
